...
import autopy
from ctypes import cast, POINTER  # used for volume control
       ...
       # print(mode)
        ...
        if len(lmList) != 0:
            if fingers[-1] == 1:
                ...
                cv2.rectangle(img, (30, int(volBar)), (55, 400),
                              (215, 255, 127), cv2.FILLED)
                cv2.putText(img, f'{int(volPer)}%', (25, 430),
                            cv2.FONT_HERSHEY_COMPLEX, 0.9, (209, 206, 0), 3)


################# Cursor control Funtionality ##############################
    if mode == 'Cursor':
        active = 1
        # print(mode)
        putText(mode)
