...
class handDetector():
    def __int__(self, mode=False, maxHands=2, complexity=1, detectionCon = 0.5, trackCon = 0.5 ):
        ...
        self.hands = self.mpHands.Hands(self.mode, self.maxHands, self.complexity, self.detectionCon, self.trackCon)  # parameters already difined
    ...
    def findHands(self, img, draw=True):
        ...
        if results.multi_hand_landmarks:
            for handlms in results.multi_hand_landmarks:
                if draw:
                    self.mpDrawLine.draw_landmarks(img, handlms, self.mpHands.HAND_CONNECTIONS)  # for single hand
